...
def get_data(files, sigma = 6000):
    ...
    for fi in files:
        ...
        try:
            ...
            print('no anatomy in ', fi)
    good_cells = np.where(np.bincount(good_cells)==num_an)[0]
...
def get_cross(mouse_sess, data_dir = 'giocomo_figures0/', good_cells = [], indsnull = [], files = []):
    ...
    if len(crosscorrfile)>0:
        ...
        np.savez(data_dir + '/' + mouse_sess + '_crosscorr_train2', 
            crosscorr_train = crosscorr_train)
...
def get_ind(mouse_sess, data_dir = 'giocomo_figures_final12', files = [], good_cells = [], indsnull = [], posxx = [], nbs = -1, lentmp = 0):
    ...
    if len(indfile)>0:
        ...
        ################### Get crosscorr stats ####################  
        crosscorr_train = get_cross(mouse_sess, files = files,good_cells = good_cells, indsnull = indsnull)
        num_neurons = len(good_cells)
...
def get_coords_ds(sspikes2, rips_real, times_cube, indstemp, ph_classes = [0,1], dec_thresh = 0.99, coeff = 47, dim = 7):
    num_circ = len(ph_classes)
    ...
    for j,c in enumerate(ph_classes):
        cocycle = cocycles[iMax[-(c+1)]]
        threshold = births1[iMax[-(c+1)]] + (deaths1[iMax[-(c+1)]] - births1[iMax[-(c+1)]])*dec_thresh
        coordstemp,inds = get_coords(cocycle, threshold, len(indstemp), dists_land, coeff)
        coords1[j,inds] = coordstemp
...
def get_coords_all(sspikes2, rips_real, times_cube, indstemp, 
    ph_classes = [0,1], dec_thresh = 0.99, coeff = 47, dim = 7, bPred = False):
    num_circ = len(ph_classes)
    ...
    spkscale = (sspikes2-spkmean)/spkstd
    dspk1 = spkscale.copy()
    __, e1, e2 = pca(spkscale[times_cube,:], dim = dim)
    ...
    coords_mod1 = np.arctan2(mtot2,mtot1)%(2*np.pi)
    if bPred:
        coords_mod2 = np.zeros_like(coords_mod1[:,:])
        coords_mod2[:,0] = predict_color(coords1[0,:], dspk1, dspk, 
                                     dist_measure='cosine',  k = 30)
        coords_mod2[:,1] = predict_color(coords1[1,:],  dspk1, dspk,
                                         dist_measure='cosine',  k = 30)
        return coords_mod1, coords1, coords_mod2
    else:
        return coords_mod1, coords1
...
def radial_downsampling1(data_in, epsilon = 0.1, metric = 'euclidean', startindex = -1):    
    n = data_in.shape[0]
    np.random.seed(0) 
    if epsilon > 0:
        n = data_in.shape[0]
        ...
        while j < n+1:
            disttemp = (cdist(data_in[i, :].reshape(1, -1), data_in[inds1, :], metric=metric) - epsilon)[0]                        
            dists[inds1] = np.min(np.concatenate((dists[inds1][:,np.newaxis], disttemp[:,np.newaxis]),1),1)
            inds[i] = j
            inds1 = inds1[disttemp>0]
            j = j+1
            if len(inds1)>0:
...
def get_ind(mouse_sess, data_dir = 'giocomo_figures_final12', indsnull = [], posxx = [], nbs = -1, lentmp = 0):
    indfile = glob.glob(data_dir + '/' + mouse_sess + '_mods.npz')
    if len(indfile)>0:
        f = np.load(indfile[0], allow_pickle = True)
        ind = f['ind'][()]
